...
public class CartController {
	...
	public CartData getCart(@ApiParam(value = "User", required = true) @Valid @PathVariable final String user) {
		return cartService.getCart(user);// skaitome per DAO
	}
	...
	public void addProductToCart(@ApiParam(value = "User", required = true) @Valid @PathVariable final String user,
		...
		cartService.addProductToCart(user, cmd.getId(), cmd.getTitle(), cmd.getQuantity());
		System.out.println("-------------------- (kontroleris) Gautas prekės id " + cmd.getId());
